...
func TestPartialCacheWithoutQuarantineIsRebuilt(t *testing.T) {
	clearBuiltinKeys(t)
	dir := t.TempDir()
	manifestPath := filepath.Join(dir, "free-models.json")
	cachePath := filepath.Join(dir, "models.json")
	...
	if got := len(reloaded.Models()); got != 2 {
		t.Fatalf("partial cache was accepted: models=%#v", reloaded.Models())
	}
}
