...
const modelTypes = ['normal','embedding','audio','image','video','rerank','moderation'];
let dirty = false;
...
function effectiveModel(model) {
  ...
  return {...model, disabled:Boolean(override.disabled), type:override.type || model.type, capabilities:{...model.capabilities, tool_call:override.tool_call ?? model.capabilities?.tool_call, vision:override.vision ?? model.capabilities?.vision, reasoning:override.reasoning ?? model.capabilities?.reasoning}};
}
...
function eligible(route) {
  return state.models.map(effectiveModel).filter(model => !model.disabled && model.type === route.type && (!route.require_tool || model.capabilities?.tool_call));
}
...
function renderRoutes() {
  ...
  $('#route-grid').innerHTML = Object.entries(state.config.routes).sort(([a],[b]) => a.localeCompare(b)).map(([alias, route]) => {
    ...
    const rows = route.models.map((id, index) => {
      ...
      const unavailable = !model || model.disabled || model.type !== route.type || (route.require_tool && !model.capabilities?.tool_call);
    ...
    return `<article class="route-card"><div class="route-head"><div><h3>${esc(alias)}</h3><p>${esc(labels[alias] || alias)} · 客户端 model 字符串</p></div><span class="type-pill ${route.require_tool?'tools':''}">${esc(route.type)}${route.require_tool?' + tools':''}</span></div><div class="fallback-list">${rows || '<div class="auto-route"><b>AUTO</b> 根据能力、禁用状态和健康度动态选择</div>'}</div><div class="add-model"><select aria-label="为 ${esc(alias)} 添加模型"><option value="">选择缓存模型…</option>${options}</select><button data-add="${esc(alias)}">添加</button></div></article>`;
  }).join('');
  bindRouteEvents();
}
...
function renderTypeFilter() {
  const select=$('#type-filter'), current=select.value, types=[...new Set(state.models.map(model=>effectiveModel(model).type))].sort();
...
function renderModels() {
  ...
  const models=state.models.map(model=>({raw:model,effective:effectiveModel(model)})).filter(({effective:model})=>(!type||model.type===type)&&(!query||model.id.toLowerCase().includes(query)||model.provider.toLowerCase().includes(query)));
  $('#model-table').innerHTML=models.map(({raw,effective:model})=>{
    ...
    return `<tr class="${model.disabled?'disabled':''}"><td class="table-model"><strong>${esc(model.id)}</strong><small>${esc(model.name||model.upstream_id)}</small></td><td><span class="type-pill">${esc(model.type)}</span></td><td><div class="caps">${caps.join('')||'<span class="cap">standard</span>'}</div></td><td><span class="health-pill ${esc(health.status)}" title="${esc(health.last_error||'')}">${esc(health.status)}</span></td><td>${model.context_length?Number(model.context_length).toLocaleString():'—'}</td><td>${esc(model.provider)}</td><td><details class="override-details"><summary>设置</summary><div class="override-panel"><label>类型<select data-override="type" data-model="${esc(model.id)}">${typeOptions}</select></label>${triStateSelect(raw,'tool_call',override.tool_call)}${triStateSelect(raw,'vision',override.vision)}${triStateSelect(raw,'reasoning',override.reasoning)}<button class="${model.disabled?'':'secondary'}" data-toggle-model="${esc(model.id)}">${model.disabled?'重新启用':'禁用模型'}</button></div></details></td></tr>`;
